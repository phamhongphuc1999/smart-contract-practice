...
import { ETH_1, HashZero, createAccount, createAccountOwner, getBalance } from '../utils';
import { fillUserOpDefault, getUserOpHash, signUserOp } from '../utils/UserOp';
import { UserOperation } from '../utils/user-operation';
...
describe('Account test', function () {
  ...
  describe('#validateUserOp', async () => {
    ...
    it('Should reject same tx on nonce error', async () => {
      await expect(account.validateUserOp(userOp, userOpHash, 0)).to.revertedWith(
        'Account::validateUserOp:invalid nonce'
      );
    });
  ...
  describe('Execution', function () {
    it('Call a view function', async function () {
      ...
      let callData = accountInter.encodeFunctionData('getNonce', []);
      let result = await account.staticExecute(account.address, callData);
      let decodedResult = accountInter.decodeFunctionResult('getNonce', result);
